public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(funcidx(cmdfunction) == -1) // If it doesn't exist, all hope is not lost! It might be defined as an admin command which has the admin level after the command name
	{
		new
			...
			iLoop = 3; // The highest admin level

		while(iLoop > 0) // Loop backwards through admin levels, from 3 to 1
		{
...
*/
	...
	if		(result == 0) SendClientMessage(playerid, ORANGE, " >  That is not a recognized command. Check the "C_BLUE"/help "C_ORANGE"dialog.");
	else if	(result == 1) return 1; // valid command, do nothing.
	else if	(result == 2) SendClientMessage(playerid, ORANGE, " >  You cannot use that command right now.");
	else if	(result == 3) SendClientMessage(playerid, RED, " >  You cannot use that command on that player right now.");
	else if	(result == 4) SendClientMessage(playerid, RED, " >  Invalid ID");
	else if	(result == 5) SendClientMessage(playerid, RED, " >  You have insufficient authority to use that command.");
	else if	(result == 6) SendClientMessage(playerid, RED, " >  You can only use that command while on "C_BLUE"administrator duty"C_RED".");
